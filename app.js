...
//app.use(checkAuth);
app.use(cors({ origin: "*" })); // Cho phép tất cả nguồn
...
app.post("/api/save-notification", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error saving notification", error: error.message });
  }
});
...
app.post("/save-printer", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      user.set("printers", [
        ...user.printers,
        { name, ip, type, port, index: user.printers.length },
      ]);
...
app.put("/api/update-printer/:printerId", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      const printerIndex = user.printers.findIndex(
        (printer) => printer.index.toString() === printerId
      );
      if (printerIndex !== -1) {
        // Cập nhật thông tin máy in
        user.printers[printerIndex] = {
          name,
          ip,
          type,
          port,
    ...
    res
      .status(500)
      .json({ message: "Error updating printer", error: error.message });
  }
});
...
app.get("/shopeefood/orders", async (req, res) => {
  ...
  const headers = {
    ...
    "SPC-B-OFT":
      "",
